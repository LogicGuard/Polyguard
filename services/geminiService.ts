...
import {
  ...
  ChatMessage,
  PortfolioSnapshot,
  StakingAnalysisResult,
  TransactionAnalysisResult,
  DeFiPortfolioAnalysis,
  NFTAnalysisResult,
  SmartContractAuditResult,
  DAppCertificationResult,
  GasOptimizationResult,
  EcosystemHealthResult,
  PerformanceAnalysisResult,
  DAOProposalAnalysisResult,
...
const setCoolingDown = (value: boolean) => {
  ...
  systemStatusEvents.dispatchEvent(
    new CustomEvent('statusChange', { detail: { isCoolingDown: value } })
  );
};
...
const handleGeminiError = (error: any): { data: null; error: string } => {
  ...
  if (isRateLimit) {
    ...
    return {
      data: null,
      error: 'QUOTA_EXHAUSTED: Security kernel recalibrating. Normal operations resume in 60s.',
    };
  }
...
async function analyzeWithStructuredSchema<T>(
  prompt: string,
  schema: any,
  cacheKey?: string,
...
export const connectToLiveAssistant = (callbacks: any) => {
  ...
  return ai.live.connect({
    ...
    config: {
      ...
      systemInstruction:
        'You are PolyGuard Specialist, a highly technical crypto security AI. Concisely analyze Polygon network threats. Yielding priority to mission critical data streams.',
...
export async function getTokenApprovals(
  address: string
): Promise<{ data: TokenApproval[] | null; error: string | null }> {
  ...
  return analyzeWithStructuredSchema<TokenApproval[]>(
    prompt,
    schema,
    `approvals_${address}`,
    TTL.SHORT,
...
export async function getPortfolioSnapshot(
  address: string
): Promise<{ data: PortfolioSnapshot | null; error: string | null }> {
  ...
  return analyzeWithStructuredSchema<PortfolioSnapshot>(
    prompt,
    schema,
    `snapshot_${address}`,
    TTL.MEDIUM,
...
export async function getIntelligenceBriefing(): Promise<{
...
export async function getSecurityAlerts(): Promise<{
  ...
  return analyzeWithStructuredSchema<SecurityAlert[]>(
    prompt,
    schema,
    'security_alerts',
    TTL.MEDIUM,
...
export async function getOnChainEvents(): Promise<{
  data: OnChainEvent[] | null;
  error: string | null;
}> {
  ...
  return analyzeWithStructuredSchema<OnChainEvent[]>(
    prompt,
    schema,
    'onchain_events',
    TTL.MEDIUM,
...
export async function analyzeWalletReport(
  address: string
): Promise<{ data: WalletReportResult | null; error: string | null }> {
  ...
  return analyzeWithStructuredSchema<WalletReportResult>(
    prompt,
    schema,
    `wallet_report_${address}`,
    TTL.MEDIUM,
    false
  );
}
...
export async function analyzeBridgeSecurity(
  address: string
): Promise<{ data: BridgeSecurityResult | null; error: string | null }> {
  try {
  ...
  const schema = {
    ...
    properties: {
      ...
      withdrawalSafety: {
        type: Type.OBJECT,
        properties: { risk: { type: Type.STRING }, summary: { type: Type.STRING } },
  ...
  return analyzeWithStructuredSchema<BridgeSecurityResult>(
    prompt,
    schema,
    undefined,
    TTL.MEDIUM,
...
export async function analyzeRegulatoryCompliance(
  identifier: string
): Promise<{ data: RegulatoryComplianceResult | null; error: string | null }> {
  ...
  return analyzeWithStructuredSchema<RegulatoryComplianceResult>(
    prompt,
    schema,
    undefined,
    TTL.MEDIUM,
...
export async function analyzeTransactionWithFirewall(
  targetContract: string,
  txData: string
): Promise<{ data: FirewallAnalysisResult | null; error: string | null }> {
  try {
    const { apiService } = await import('./apiService');
    const sim = await apiService.simulateFirewallTx(txData, 0);
    if (sim && sim.firewall_verdict) {
      const status: 'Allowed' | 'Blocked' =
        sim.simulation_status === 'BLOCKED' ? 'Blocked' : 'Allowed';
      return {
        data: {
          status,
  ...
  return analyzeWithStructuredSchema<FirewallAnalysisResult>(
    prompt,
    schema,
    undefined,
    TTL.SHORT,
...
export async function analyzeTransaction(
  txData: string
): Promise<{ data: TransactionAnalysisResult | null; error: string | null }> {
  ...
  const schema = {
    ...
    properties: {
      ...
      transactionFlow: {
        ...
        required: ['from', 'to', 'value', 'action'],
      },
    },
    required: ['riskLevel', 'summary', 'warnings', 'transactionFlow'],
  };
  return analyzeWithStructuredSchema<TransactionAnalysisResult>(
    prompt,
    schema,
    undefined,
    TTL.SHORT,
...
export async function analyzeQuantumResistance(
  address: string
): Promise<{ data: QuantumAnalysisResult | null; error: string | null }> {
  ...
  const schema = {
    ...
    properties: {
      ...
      vulnerableComponents: {
        ...
        items: {
          type: Type.OBJECT,
          properties: { component: { type: Type.STRING }, detail: { type: Type.STRING } },
      ...
      pqcRecommendations: {
        ...
        items: {
          type: Type.OBJECT,
          properties: { algorithm: { type: Type.STRING }, useCase: { type: Type.STRING } },
          required: ['algorithm', 'useCase'],
        },
      },
  ...
  return analyzeWithStructuredSchema<QuantumAnalysisResult>(
    prompt,
    schema,
    undefined,
    TTL.MEDIUM,
...
export async function simulateZKProofVerification(
  address: string
): Promise<{ data: ZKProofVerificationResult | null; error: string | null }> {
  try {
    const { apiService } = await import('./apiService');
  ...
  const schema = {
    ...
    properties: {
      ...
      verifiedClaims: {
        ...
        items: {
          type: Type.OBJECT,
          properties: { claim: { type: Type.STRING }, status: { type: Type.STRING } },
  ...
  return analyzeWithStructuredSchema<ZKProofVerificationResult>(
    prompt,
    schema,
    undefined,
    TTL.SHORT,
...
export async function getNamesForAddresses(
  addresses: string[]
): Promise<{
  ...
  const result = await analyzeWithStructuredSchema<{ results: any[] }>(
    prompt,
    schema,
    `names_${addresses.join('_')}`,
    TTL.LONG,
    true
  );
  if (result.data) {
    ...
    result.data.results.forEach((item) => {
      mapped[item.address] = { name: item.name, symbol: item.symbol };
    });
...
export async function analyzeSmartContractAudit(
  code: string
): Promise<{ data: SmartContractAuditResult | null; error: string | null }> {
  try {
    const { apiService } = await import('./apiService');
    const apiRes = await apiService.runContractAudit(code);
    if (apiRes && apiRes.audit_id) {
      const riskLevel: 'Low' | 'Medium' | 'High' | 'Critical' =
        apiRes.overall_status === 'CRITICAL'
          ? 'Critical'
          : apiRes.overall_status === 'VULNERABLE'
            ? 'High'
            : 'Low';
  ...
  return analyzeWithStructuredSchema<SmartContractAuditResult>(
    prompt,
    schema,
    undefined,
    TTL.MEDIUM,
