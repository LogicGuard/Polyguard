...
logger = logging.getLogger("polyguard.ontology")

class PolyGuardOntology:
    def __init__(self):
        self.graph = Graph()
        self.pg = Namespace("https://polyguard.ai/ontology#")
    ...
    def _build_ontology(self):
        ...
        logger.info(f"PolyGuard Ontology initialized with {len(self.graph)} triples.")

    def add_vulnerability_instance(self, vuln_id: str, vuln_type: str, severity: str, contract_addr: str):
        vuln_uri = URIRef(f"https://polyguard.ai/ontology#Vuln_{vuln_id}")
        contract_uri = URIRef(f"https://polyguard.ai/ontology#Contract_{contract_addr}")
...
ontology_engine = PolyGuardOntology()
