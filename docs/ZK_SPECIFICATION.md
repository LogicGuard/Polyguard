# PolyGuard Zero-Knowledge Cryptographic & Pessimistic Prover Specification
...
If $\sum_{i=1}^{M} W_{k,i} > \sum_{j=1}^{P} D_{k,j}$, the proof generation fails, and PolyGuard immediately issues a bridge pause signal.
...
Below is the production Circom circuit used by PolyGuard for Zero-Knowledge Solvency & Sanctions List Verification:
...
template PolyGuardPessimisticInvariant(levels) {
    // Inputs
