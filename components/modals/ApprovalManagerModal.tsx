...
const ApprovalManagerModal: React.FC<ApprovalManagerModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleRevoke = async (tokenAddr: string, spenderAddr: string) => {
    ...
    setApprovals((prev) =>
      prev.filter(
        (appr) => !(appr.tokenAddress === tokenAddr && appr.spenderAddress === spenderAddr)
      )
    );
    setIsRevoking(null);
  };
            ...
            <Card className="w-full flex flex-col max-h-[90vh] bg-[#0C0C0C] border border-white/10 shadow-2xl p-0 overflow-hidden">
              <div className="p-4 lg:p-5 border-b border-white/10 bg-[#080808] flex justify-between items-center flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base lg:text-lg font-bold text-white uppercase tracking-wider">
                      Permits
                    </h2>
              ...
              <div className="p-4 lg:p-6 overflow-y-auto custom-scrollbar flex-1 bg-[#030303]">
                        ...
                        >
                          ...
                          <div className="flex gap-2 md:col-span-2">
                            ...
                            <Button
                              ...
                              className="text-[8px] whitespace-nowrap px-2"
                            >
                              Unlimited
                            </Button>
                          </div>
                          <div className="md:col-span-2 text-right">
                            <Button type="submit" disabled={isGranting} className="text-[9px] px-8">
                              Grant_Permit
                            </Button>
                          </div>
                        </form>
                ...
                {isLoading && (
                  <div className="space-y-2 mt-2">
                    {[...Array(3)].map((_, i) => (
                      <SkeletonLoader key={i} className="h-14 w-full rounded-none" />
                    ))}
                  </div>
                )}
                  ...
                  <div className="mt-0 lg:mt-2 space-y-2 lg:space-y-0">
                    {approvals.map((approval, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex flex-col lg:grid lg:grid-cols-12 gap-2 lg:gap-4 px-4 py-4 border lg:border-none border-white/5 lg:border-b bg-white/[0.02] lg:bg-transparent items-center hover:bg-white/[0.04] transition-colors rounded-sm lg:rounded-none"
                        >
                          ...
                          <div className="col-span-4 w-full lg:w-auto text-left">
                            <div className="text-xs text-gray-300 truncate">
                              <span className="lg:hidden text-[9px] text-gray-600 mr-2 uppercase">
                                To:
                              </span>
                          ...
                          <div className="col-span-3 w-full lg:w-auto flex justify-between lg:block items-center mt-2 lg:mt-0">
                            <span className="lg:hidden text-[9px] text-gray-600 uppercase">
                              Exposure:
                            </span>
                            <span
                              className={`px-2 py-0.5 rounded-sm text-[8px] font-bold uppercase ${isUnlimited ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}
                            >
                          ...
                          <div className="col-span-1 w-full lg:w-auto text-right mt-3 lg:mt-0 pt-3 lg:pt-0 border-t border-white/5 lg:border-none">
                            <Button
                              variant="secondary"
                              onClick={() =>
                                handleRevoke(approval.tokenAddress, approval.spenderAddress)
                              }
                            ...
                            >
                              {isRevoking === approval.spenderAddress ? (
                                <div className="w-3 h-3 border-2 border-t-transparent border-red-400 rounded-full animate-spin"></div>
                              ) : (
                                <TrashIcon className="w-4 h-4" />
                              )}
                              <span className="lg:hidden ml-2 text-[10px] font-bold">
                                REVOKE PERMIT
                              </span>
                            </Button>
                          </div>
