...
const simpleMarkdownParser = (text: string): string => {
  ...
  const processedParts = parts.map((part, index) => {
    if (index % 2 === 1) {
      ...
      const codeContent = part
        ...
        .replace(/```$/, '')
        .trim();
      const langBadgeClass = lang.includes('RUST')
        ? 'bg-orange-500/10 border-orange-500/30 text-orange-400'
        : lang.includes('GO')
          ? 'bg-cyan-500/10 border-cyan-500/30 text-cyan-400'
          : lang.includes('SOLIDITY') || lang.includes('YUL')
            ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400'
            : lang.includes('CIRCOM')
              ? 'bg-purple-500/10 border-purple-500/30 text-purple-400'
              : lang.includes('HUFF') || lang.includes('MOVE')
    ...
    return (
      part
        ...
        .replace(/\|(.+)\|/g, (match) => {
          ...
          let tableHtml =
            '<div class="my-10 overflow-x-auto border border-white/10 rounded-none bg-black/40"><table class="w-full text-left font-mono text-[10px] uppercase tracking-tighter border-collapse">';
          rows.forEach((row, i) => {
            const cells = row
              .split('|')
              .filter((c) => c.trim() !== '' || row.indexOf('|') !== row.lastIndexOf('|'));
        ...
        .replace(
          /^### (.*$)/gim,
          '<h3 class="text-white font-mono text-sm font-black uppercase tracking-widest mt-12 mb-5 border-l-4 border-blue-500 pl-5">$1</h3>'
        )
        .replace(
          /^## (.*$)/gim,
          '<h2 class="text-white font-mono text-lg font-black uppercase tracking-[0.2em] mt-16 mb-8 border-b border-white/10 pb-4">$1</h2>'
        )
        .replace(
          /^# (.*$)/gim,
          '<h1 class="text-white font-mono text-2xl font-black uppercase tracking-[0.3em] mt-20 mb-10 text-blue-400">$1</h1>'
        )
        .replace(
          /^\* (.*$)/gim,
          '<li class="ml-6 pl-3 border-l border-white/10 hover:border-blue-500/50 transition-colors py-1.5">$1</li>'
        )
        .replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-bold">$1</strong>')
        .replace(
          /`([^`]+)`/g,
          '<code class="bg-white/10 text-blue-300 px-2 py-0.5 rounded-sm font-mono text-[11px] border border-white/5">$1</code>'
        )
        .replace(
          /\n\n/g,
          '</p><p class="mb-8 leading-relaxed text-gray-400 text-[14px] tracking-tight">'
        )
        .replace(/\n/g, '<br />')
        .replace(/^<p><\/p>$/g, '')
    );
  ...
  if (!html.startsWith('<') && !html.startsWith('Data_Payload') && !html.startsWith('<div')) {
    html =
      '<p class="mb-8 leading-relaxed text-gray-400 text-[14px] tracking-tight">' + html + '</p>';
  }
      ...
      <div className="mt-20 pt-6 border-t border-white/10 flex justify-between items-center text-[9px] font-mono text-gray-600 uppercase font-black tracking-widest">
        <div className="flex items-center gap-6">
          <span className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 bg-blue-500 rounded-full"></div> END_OF_SPECIFICATION
          </span>
        </div>
