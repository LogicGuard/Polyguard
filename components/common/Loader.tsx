...
const SkeletonLoader: React.FC<SkeletonLoaderProps> = ({ className }) => {
  return (
    <div
      className={`relative overflow-hidden bg-white/[0.03] border border-white/5 rounded-sm ${className}`}
    >
      <motion.div
...
export const ListSkeleton: React.FC<SkeletonLoaderProps & { items?: number }> = ({
  items = 5,
  className,
}) => (
...
export const GridSkeleton: React.FC<SkeletonLoaderProps & { cols?: number; rows?: number }> = ({
  cols = 3,
  rows = 1,
  className,
}) => (
  ...
  <div className={`p-4 bg-[#020202] border border-white/10 font-mono space-y-3 ${className}`}>
    {Array.from({ length: 12 }).map((_, i) => (
      <div key={i} className="flex gap-4">
        ...
        <SkeletonLoader
          className={`h-3 ${i % 3 === 0 ? 'w-3/4' : i % 2 === 0 ? 'w-1/2' : 'w-5/6'}`}
        />
      </div>
    ))}
  </div>
