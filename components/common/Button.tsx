...
const Button: React.FC<ButtonProps> = ({
  ...
  const baseStyles =
    'px-4 py-1.5 font-mono text-[10px] font-bold uppercase tracking-wider focus:outline-none transition-all duration-300 flex items-center justify-center space-x-2 relative overflow-hidden group rounded-sm';

  const variantStyles = {
    primary:
      'bg-white text-black hover:bg-gray-100 border border-transparent shadow-[0_3px_0_0_rgba(200,200,200,1)] active:translate-y-[1px] active:shadow-[0_1px_0_0_rgba(200,200,200,1)]',
    secondary:
  ...
  return (
    ...
    >
      {Icon && (
        <motion.div initial={false} animate={{ x: 0 }} whileHover={{ x: -2 }}>
