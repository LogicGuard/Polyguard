...
const PrimarySidebar: React.FC<PrimarySidebarProps> = ({
  ...
  const [activeOverlay, setActiveOverlay] = useState<
    'settings' | 'alerts' | 'terminal' | 'profile' | null
  >(null);
        ...
        <div className="w-full p-6 space-y-6 relative z-10">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-1.5 px-2 py-0.5 bg-blue-500/10 border border-blue-500/20 rounded-none shadow-[0_0_10px_rgba(59,130,246,0.1)]">
              ...
              <span className="text-[7px] font-mono text-blue-400 font-black uppercase tracking-widest">
                S1_Operational
              </span>
            </div>
          </div>
          ...
          <div className="hidden lg:block relative group/search">
            <div
              className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors duration-300 ${isSearchFocused ? 'text-blue-500' : 'text-gray-600'}`}
            >
            ...
            <AnimatePresence>
              {isSearchFocused && searchQuery.length > 0 && (
                ...
                >
                  <div className="text-[7px] font-mono text-gray-600 uppercase mb-2 px-2">
                    Heuristic_Predictions
                  </div>
                  {['AUDIT_CONTRACT', 'SCAN_MEMPOOL', 'NETWORK_TRAFFIC']
                    .filter((c) => c.toLowerCase().includes(searchQuery.toLowerCase()))
                    .map((cmd, i) => (
                      <button
                        key={i}
                        onClick={() => handleExecuteCmd(cmd)}
                        className="w-full text-left p-2 hover:bg-white/5 text-[8px] font-mono text-gray-400 hover:text-blue-400 transition-colors uppercase font-black"
                      >
                        &gt; {cmd}
                      </button>
                    ))}
          ...
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="relative">
                ...
                >
                  <div
                    className={`relative z-10 flex items-center justify-center transition-all duration-500 ${isActive ? 'text-blue-400 scale-110' : 'group-hover:text-white'}`}
                  >
                  ...
                  <div className="hidden lg:flex flex-col flex-1 z-10 min-w-0">
                    <span
                      className={`font-mono text-[10px] uppercase tracking-[0.2em] font-black transition-all ${isActive ? 'translate-x-1' : 'opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5'}`}
                    >
                      {item.label}
                    </span>
                  </div>
                  ...
                  {hasSubItems && (
                    <ChevronDownIcon
                      className={`hidden lg:block w-3 h-3 text-gray-700 transition-transform duration-300 z-10 ${isActive ? 'rotate-180' : ''}`}
                    />
                  )}
                </button>
                    ...
                    >
                      <div className="pl-11 py-2 border-l border-white/5 ml-5 space-y-1">
                        {item.subItems?.map((sub) => (
                          ...
                          >
                            <div
                              className={`w-1 h-1 rounded-none rotate-45 transition-all ${activeSubItem === sub.id ? 'bg-blue-400 scale-150 shadow-[0_0_8px_#3b82f6]' : 'bg-gray-800'}`}
                            ></div>
                            <span
                              className={`text-[8px] font-mono uppercase tracking-widest font-black hidden lg:block ${activeSubItem === sub.id ? 'text-blue-400' : 'text-gray-600 hover:text-gray-400'}`}
                            >
                              {sub.label}
                            </span>
                          </button>
            ...
            {[
              ...
              >
                {activeOverlay === tool.id && (
                  <motion.div
                    layoutId="toolActiveGlow"
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 shadow-[0_0_10px_#3b82f6]"
                  />
                )}
                <tool.icon
                  className={`w-3.5 h-3.5 transition-colors duration-300 ${activeOverlay === tool.id ? 'text-blue-400 scale-110' : 'text-gray-600 group-hover:text-blue-400'}`}
                />
                <span
                  className={`hidden lg:block text-[6px] font-mono mt-1.5 uppercase font-black transition-colors ${activeOverlay === tool.id ? 'text-white' : 'text-gray-700 group-hover:text-gray-400'}`}
                >
                  {tool.id}
                </span>
              </button>
