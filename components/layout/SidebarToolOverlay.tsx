...
const SidebarToolOverlay: React.FC<SidebarToolOverlayProps> = ({ activeTool, onClose }) => {
  ...
  const handleCommand = async (e: React.FormEvent) => {
    ...
    setLogs((prev) => [
      ...prev,
      { t: new Date().toLocaleTimeString(), m: `> ${commandValue.toUpperCase()}`, s: 'info' },
    ]);
    setCommandValue('');
    setIsThinking(true);
    ...
    <div className="flex flex-col h-full bg-black/20 font-mono text-[11px]">
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-1">
        {logs.map((log, i) => (
          <div
            key={i}
            className="flex gap-3 hover:bg-white/[0.03] transition-colors pl-2 py-0.5 border-l border-transparent hover:border-white/5"
          >
      ...
      <form
        ...
        className="p-2 border-t border-white/5 bg-black/40 flex items-center gap-3 px-4 group"
      >
        <span className="text-blue-500 font-black text-xs group-focus-within:animate-pulse">
          {'>'}
        </span>
        <input
          type="text"
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-full overflow-hidden">
      <div className="flex flex-col gap-2 overflow-y-auto custom-scrollbar pr-2 h-full">
        {[
          {
            id: 1,
            m: 'MALICIOUS_HANDSHAKE_DETECTION',
            s: 'CRIT',
            t: '14:02',
            details: 'Source: 0x921...E4. Method: Recursive Call Bypass attempt on Bridge V2.',
          },
          {
            id: 2,
            m: 'MEMPOOL_THRESHOLD_GAS_SPIKE',
            s: 'WARN',
            t: '13:55',
            details: 'Congestion detected on Polygon Mainnet RPC. Potential MEV attack signature.',
          },
          {
            id: 3,
            m: 'KERNEL_V4_CALIBRATION_OK',
            s: 'SYS',
            t: '12:12',
            details: 'Self-check complete. Heuristic confidence: 99.82%.',
          },
        ].map((alert) => (
          <div
            key={alert.id}
          ...
          >
            <div
              className={`absolute left-0 top-0 bottom-0 w-1 ${alert.s === 'CRIT' ? 'bg-red-500' : alert.s === 'WARN' ? 'bg-yellow-500' : 'bg-blue-500'}`}
            ></div>
            <div className="flex justify-between mb-1">
              <span className="text-[10px] font-mono font-black text-white truncate pr-2 uppercase tracking-tighter">
                {alert.m}
              </span>
      ...
      <div className="bg-white/[0.02] border border-white/10 p-4 font-mono flex flex-col relative overflow-hidden">
        ...
        <h4 className="text-[9px] font-black text-blue-500 uppercase mb-4 tracking-widest border-b border-white/5 pb-2">
          Forensic_Analysis
        </h4>
        <AnimatePresence mode="wait">
          {selectedAlert ? (
            ...
            >
              <div className="text-[10px] text-gray-300 leading-relaxed uppercase">
                {
                  [
                    {
                      id: 1,
                      d: 'Source: 0x921...E4. Method: Recursive Call Bypass attempt on Bridge V2.',
                    },
                    {
                      id: 2,
                      d: 'Congestion detected on Polygon Mainnet RPC. Potential MEV attack signature.',
                    },
              ...
              <Button variant="secondary" className="w-full !text-[8px] py-1">
                GENERATE_DOCKET
              </Button>
            </motion.div>
          ) : (
            <div className="h-full flex items-center justify-center text-[9px] text-gray-700 uppercase italic">
              Select incident for forensics...
            </div>
          )}
        </AnimatePresence>
      </div>
  ...
  const renderSettings = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-2 h-full overflow-y-auto custom-scrollbar">
      <div className="space-y-6">
        <div>
          <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 border-b border-white/5 pb-1">
            Uplink_Controls
          </h3>
          <div className="space-y-3">
            {[
              { id: 'stealthMode', label: 'STEALTH_SIGNAL_MODE', desc: 'Mask RPC node footprints' },
              {
                ...
                label: 'DEEP_PACKET_INSPECT',
                desc: 'Full heuristic parsing',
              },
              {
                id: 'aggLayerSync',
                label: 'AGGLAYER_REALTIME',
                desc: 'Zero-latency state tracking',
              },
            ].map((s) => (
              <div
                key={s.id}
                className="flex items-center justify-between p-3 bg-white/[0.02] border border-white/5 group hover:border-blue-500/20 transition-all"
              >
                ...
                <button
                  onClick={() =>
                    setSettings((prev) => ({
                      ...prev,
                      [s.id]: !prev[s.id as keyof typeof settings],
                    }))
                  }
        ...
        <div>
          ...
          <Card className="p-4 bg-black/40 border-white/5">
            ...
            <input
              ...
              onChange={(e) =>
                setSettings((prev) => ({ ...prev, alertThreshold: parseInt(e.target.value) }))
              }
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-1.5 border-b border-white/5 bg-[#0A0A0A]/80 flex-shrink-0">
            <div className="flex items-center gap-1 overflow-x-auto no-scrollbar">
              {tabs.map((tab) => (
                ...
                >
                  <tab.icon
                    className={`w-3.5 h-3.5 ${currentTab === tab.id ? tab.color : 'opacity-40'}`}
                  />
                  <span>{tab.label}</span>
                  {currentTab === tab.id && (
                    <motion.div
                      layoutId="tabUnderline"
                      className="absolute bottom-0 left-2 right-2 h-0.5 bg-blue-500 shadow-[0_0_10px_#3b82f6]"
                    />
                  )}
                </button>
              ))}
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center gap-4 border-l border-white/10 pl-4 h-6">
                <div className="flex flex-col items-end">
                  <span className="text-[7px] text-gray-600 uppercase font-black">
                    AI_INFERENCE_LOAD
                  </span>
              ...
              <button
                onClick={onClose}
                className="p-1.5 hover:bg-white/5 text-gray-500 hover:text-white transition-all rounded-sm"
              >
              ...
              >
                ...
                {currentTab === 'profile' && (
                  <div className="flex flex-col items-center justify-center h-full max-w-sm mx-auto text-center space-y-6">
                    ...
                    <div>
                      <h2 className="text-sm font-mono font-black text-white uppercase tracking-widest mb-1">
                        Operator_Alpha_01
                      </h2>
                      <p className="text-[9px] text-gray-500 font-mono uppercase">
                        Level: Institutional_Sovereign // ID: OP_9912
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-4 w-full">
                      <div className="p-3 border border-white/5 bg-white/[0.01]">
                        <div className="text-[7px] text-gray-600 uppercase font-black mb-1">
                          Trust_Index
                        </div>
          ...
          <div className="px-6 py-1 border-t border-white/5 bg-[#050505] flex justify-between items-center text-[7px] font-mono text-gray-700 uppercase tracking-widest flex-shrink-0">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-blue-500/40 font-black">POLYGUARD_DOCK_V4</span>
            </div>
          </div>
        </motion.div>
      )}
