...
const NotificationCenter: React.FC<NotificationCenterProps> = ({
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'Security':
        return <ThreatIcon className="w-4 h-4 text-red-400" />;
      case 'System':
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="p-3 border-b border-white/10 flex justify-between items-center bg-[#0A0A0A]">
            <h3 className="font-bold text-white text-xs uppercase tracking-widest font-mono">
              Alert Feed
            </h3>
            <div className="flex gap-3">
              <button
                onClick={onMarkAllRead}
                className="text-[9px] font-mono text-gray-500 hover:text-white transition-colors uppercase"
              >
                READ_ALL
              </button>
              ...
              <button
                onClick={onClearAll}
                className="text-[9px] font-mono text-gray-500 hover:text-red-400 transition-colors uppercase"
              >
                PURGE
              </button>
            </div>
              ...
              notifications.map((notification) => (
                ...
                >
                  ...
                  <div className="flex gap-3 items-start">
                    <div
                      className={`mt-0.5 p-1.5 rounded-sm border ${getTypeColor(notification.type)}`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <p
                          className={`text-xs font-bold uppercase tracking-wide truncate pr-2 ${notification.read ? 'text-gray-500' : 'text-gray-200'}`}
                        >
                          {notification.title}
                        </p>
