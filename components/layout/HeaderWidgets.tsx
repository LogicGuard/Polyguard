...
export const NetworkStats: React.FC = () => {
  ...
  return (
    <div className="flex items-center gap-0 lg:gap-2 max-w-[200px] lg:max-w-none overflow-hidden relative">
      ...
      {/* MATIC Price HUD */}
      <motion.div className="bg-[#0A0A0A] px-2 lg:px-3 py-1.5 flex items-center border-r border-white/10 flex-shrink-0">
        <div className="flex flex-col items-end leading-none">
          <span className="text-[8px] font-mono text-gray-500 uppercase">POL/USD</span>
          <div className="flex items-center gap-1">
            <span className="text-xs font-bold text-white font-mono">
              ${stats.maticPrice.toFixed(2)}
            </span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="relative flex-shrink-0" ref={gasConfigRef}>
        ...
        >
          <div className="flex flex-col items-end leading-none">
            ...
            <div className="flex items-center gap-1">
              <span
                className={`text-xs font-bold font-mono ${isGasAlertActive ? 'text-red-400' : 'text-white'}`}
              >
                {Math.round(stats.gas)}
              </span>
            </div>
          </div>
        </motion.div>
      ...
      <motion.div className="hidden lg:flex bg-[#0A0A0A] px-3 py-1.5 items-center flex-shrink-0">
        <div className="flex flex-col items-end leading-none">
...
export const GlobalSearch: React.FC = () => {
  ...
  const searchTargets = [
    ...
    },
    {
      ...
      targetPrimary: 'real-time-security',
      targetSecondary: 'on-chain-monitor',
      category: 'Monitor',
    },
    {
      name: 'Threat Intelligence',
      targetPrimary: 'real-time-security',
      targetSecondary: 'threat-intelligence',
      category: 'Intel',
    },
    {
      name: 'Wallet Report',
    ...
    {
      name: 'ZK Compliance',
      targetPrimary: 'advanced-security',
      targetSecondary: 'zk-compliance',
      category: 'Compliance',
    },
    {
      name: 'AI Specialist / Live Assistant',
      targetPrimary: 'live-assistant',
      targetSecondary: '',
      category: 'AI',
    },
  ];
        ...
        {isOpen && filtered.length > 0 && (
          ...
          >
            {filtered.map((item, idx) => (
              ...
              >
                <span>&gt; {item.name}</span>
                <span className="text-[7px] text-gray-600 uppercase border border-white/10 px-1 py-0.5">
                  {item.category}
                </span>
              </button>
            ))}
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      ...
      message: 'Interaction detected with 0x8f...2a.',
      timestamp: '2m ago',
      read: false,
    },
    {
