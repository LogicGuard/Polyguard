      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 font-mono">
            Deep Identity Scanning // Risk Profiling
          </p>
        </div>
      </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 relative">
                <div className="relative w-48 h-48 flex items-center justify-center">
                  ...
                  <div className="text-center z-10">
                    ...
                    <span
                      className={`text-xs uppercase tracking-widest font-bold mt-1 block ${getRiskColor(result.riskLevel)}`}
                    >
                      {result.riskLevel}
                    </span>
                  </div>
              ...
              <Card className="p-0 bg-[#080808]">
                <div className="p-4 border-b border-white/10 bg-[#0A0A0A] flex justify-between items-center">
                  ...
                  <span className="text-[10px] font-mono text-gray-600">
                    COUNT: {result.positivePoints.length}
                  </span>
                </div>
                <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                  {result.positivePoints.map((point, i) => (
                    <div
                      key={i}
                      className="p-3 bg-green-500/5 border border-green-500/10 rounded-sm"
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <div className="w-1.5 h-1.5 bg-green-500 rounded-sm"></div>
                        <strong className="text-xs font-bold text-green-100 uppercase">
                          {point.title}
                        </strong>
                      </div>
                      <p className="text-[11px] text-green-400/70 font-mono pl-3.5">
                        {point.detail}
                      </p>
                    </div>
                  ))}
              ...
              <Card className="p-0 bg-[#080808]">
                ...
                <div className="p-4 space-y-3">
                  {result.risks.map((risk, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 p-3 bg-white/5 border border-white/5 hover:border-red-500/30 transition-colors group"
                    >
                      <div
                        className={`mt-1 px-1.5 py-0.5 text-[9px] font-bold uppercase rounded-sm border ${risk.severity === 'High' ? 'bg-red-500/20 text-red-400 border-red-500/30' : risk.severity === 'Medium' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' : 'bg-blue-500/20 text-blue-400 border-blue-500/30'}`}
                      >
                        {risk.severity}
                      </div>
                      <div>
                        <strong className="text-sm text-gray-200 block mb-1 group-hover:text-white transition-colors">
                          {risk.title}
                        </strong>
