...
const NODES = [
  {
    id: 'pos_core',
    label: 'Polygon PoS Cluster',
    type: 'core',
    x: 400,
    y: 250,
    health: 99,
    load: 42,
    info: 'Primary settlement layer. 100+ Validators active.',
  },
  {
    id: 'zkevm',
    label: 'zkEVM Prover',
    ...
    health: 100,
    load: 18,
    info: 'Zero-knowledge proof generation unit. Latency: 4ms.',
  },
  {
    id: 'agglayer',
    label: 'AggLayer V1',
    ...
    health: 98,
    load: 12,
    info: 'Unified state and liquidity orchestration layer.',
  },
  {
    id: 'bridge_eth',
    label: 'L1 Bridge Hub',
    ...
    health: 92,
    load: 75,
    info: 'High-volume liquidity corridor to Ethereum Mainnet.',
  },
  {
    id: 'mempool',
    label: 'Global Mempool',
    type: 'infra',
    x: 150,
    y: 200,
    health: 96,
    load: 55,
    info: 'Transaction propagation network. Zero congestion.',
  },
  {
    id: 'aave',
    label: 'Aave V3 Protocol',
    type: 'app',
    x: 200,
    y: 350,
    health: 97,
    load: 30,
    info: 'Liquidity market. TVL: $420M+ on Polygon.',
  },
  {
    id: 'uniswap',
    label: 'Uniswap V3',
    ...
    health: 99,
    load: 25,
    info: 'DEX state. Active pairs: 1,400+.',
  },
  {
    id: 'open_sea',
    label: 'Marketplace Relay',
    type: 'app',
    x: 700,
    y: 100,
    health: 95,
    load: 60,
    info: 'NFT trading signals and metadata indexing.',
  },
];
...
const EcosystemHealthView: React.FC = () => {
  ...
  const visibleNodes = useMemo(
    () => NODES.filter((n) => activeLayers.includes(n.type)),
    [activeLayers]
  );
    ...
    <div className="h-[calc(100vh-140px)] flex flex-col max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black uppercase tracking-widest text-white leading-none">
              Tactical Topology Matrix
            </h1>
            <p className="text-[10px] text-gray-500 font-mono mt-1 uppercase">
              Real-Time Infrastructure Visualization // Node-Level Intelligence
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
        ...
        <Card className="lg:col-span-4 p-0 bg-[#080808] flex flex-col h-full overflow-hidden border-white/10">
          <div className="p-4 border-b border-white/10 bg-[#0A0A0A] flex justify-between items-center">
            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] font-mono">
              Telemetry_Output
            </h3>
            <ActivityIcon
              className={`w-3.5 h-3.5 ${isAnalyzing ? 'text-blue-500 animate-spin' : 'text-gray-800'}`}
            />
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
            <AnimatePresence mode="wait">
              {selectedNode ? (
                ...
                >
                  <div>
                    <div className="text-[8px] font-mono text-blue-500 uppercase font-black mb-1">
                      UID: {selectedNode.id}
                    </div>
                    <h2 className="text-2xl font-black text-white uppercase tracking-tighter">
                      {selectedNode.label}
                    </h2>
                    <p className="text-[10px] text-gray-500 font-mono mt-2 uppercase leading-relaxed">
                      {selectedNode.info}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-white/[0.02] border border-white/5">
                      <div className="text-[8px] text-gray-600 font-bold uppercase mb-1">
                        Health_Score
                      </div>
                ...
                <div className="h-full flex flex-col items-center justify-center opacity-20 text-center py-20">
                  ...
                  <h4 className="text-xs font-mono uppercase tracking-[0.4em] text-white">
                    Select_Topology_Unit
                  </h4>
                  <p className="text-[10px] font-mono text-gray-500 mt-2">
                    Interact with the map to initialize deep node forensics.
                  </p>
                </div>
              )}
            </AnimatePresence>
          ...
          {selectedNode && (
            <div className="p-4 bg-[#050505] border-t border-white/10">
              <Button
                ...
                className="w-full justify-center py-4 bg-white text-black hover:bg-gray-200 uppercase font-black tracking-widest text-[10px]"
              >
                RUN_TACTICAL_AUDIT
              </Button>
            </div>
