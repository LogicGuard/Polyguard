...
const TOOLS = [
  {
    label: 'WALLET_DOSSIER',
    icon: WalletIcon,
    pid: 'asset-intelligence',
    sid: 'wallet-report',
    desc: 'Scan Identity',
  },
  {
    ...
    pid: 'real-time-security',
    sid: 'transaction-analysis',
    desc: 'Decode Signals',
  },
  {
    label: 'FIREWALL_CORE',
    icon: FirewallIcon,
    pid: 'real-time-security',
    sid: 'smart-contract-firewall',
    desc: 'Simulate Shield',
  },
  {
    label: 'AUDIT_KERNEL',
    icon: AuditorIcon,
    pid: 'security-audits',
    sid: 'smart-contract-auditor',
    desc: 'Verify Logic',
  },
];
  ...
  return (
    <Card className="bg-[#080808]/80 backdrop-blur-md p-2 border-white/5 flex items-center gap-2 shadow-2xl rounded-none relative overflow-hidden">
      ...
      <div className="px-4 border-r border-white/10 flex flex-col justify-center mr-2">
        <span className="text-[8px] font-black text-blue-500 uppercase tracking-[0.4em] font-mono">
          Operations
        </span>
        <span className="text-[10px] font-black text-white uppercase font-mono tracking-tighter">
          QUICK_START
        </span>
      </div>
        ...
        {TOOLS.map((tool, i) => (
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[9px] font-black text-white font-mono truncate uppercase tracking-widest">
                {tool.label}
              </span>
              <span className="text-[7px] font-mono text-gray-600 uppercase group-hover:text-gray-400 transition-colors">
                {tool.desc}
              </span>
            </div>
            {/* Interactive Sparkle */}
      ...
      <div className="hidden lg:flex px-4 border-l border-white/10 flex-col items-end">
        ...
        <div className="flex gap-0.5 mt-0.5">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-1 h-1 bg-green-500 rounded-full animate-pulse"></div>
          ))}
        </div>
      </div>
    </Card>
