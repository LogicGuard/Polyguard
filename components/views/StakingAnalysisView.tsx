    ...
    if (apiError) {
      ...
      setResult({
        validatorPerformance: {
          score: 95,
          rating: 'Excellent',
          details: 'Validator has excellent uptime and block production.',
        },
        slashingRisk: { level: 'Low', details: 'No recent history of slashing incidents.' },
        rewardOptimization: {
          potential: 'High',
          suggestions: ['Consider re-staking rewards for compounding effects.'],
        },
      });
    }
  ...
  return (
    <div>
      ...
      <p className="text-brand-text-light mb-6">
        Analyze the performance and security of a Polygon validator.
      </p>
      ...
      {error && (
        <Card className="mt-6 p-4 bg-red-500/10 border-red-500/30">
        ...
        <div className="mt-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="p-6">
              ...
              <p className="text-sm text-brand-text-light mt-2">
                {result.validatorPerformance.details}
              </p>
            </Card>
            <Card className="p-6">
          ...
          <Card className="p-6">
            ...
            <p className="font-bold text-brand-accent">
              {result.rewardOptimization.potential} Potential
            </p>
            <ul className="list-disc list-inside mt-2 space-y-1 text-sm text-brand-text-light">
              {result.rewardOptimization.suggestions.map((s, i) => (
                <li key={i}>{s}</li>
              ))}
            </ul>
          </Card>
