      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 font-mono">
            AML/KYC Forensics // Regulatory Alignment Check
          </p>
        </div>
      </div>
        ...
        {isLoading ? (
          <Card className="h-96 flex flex-col items-center justify-center bg-[#050505] border-white/5">
            ...
            <p className="text-[10px] font-mono text-gray-500 uppercase tracking-[0.4em]">
              Querying_International_Regulatory_Nodes...
            </p>
          </Card>
        ) : result ? (
          <motion.div
          ...
          >
            <Card className="p-6 bg-[#080808] border-white/10">
              ...
              <div
                className={`text-2xl font-black font-mono mb-2 uppercase ${getRiskColor(result.amlRisk.level)}`}
              >
                {result.amlRisk.level}_RISK
              </div>
              <p className="text-[11px] text-gray-400 font-mono leading-relaxed mb-4">
                {result.amlRisk.summary}
              </p>
              <div className="space-y-1.5">
                {result.amlRisk.flags.map((flag, i) => (
                  <div
                    key={i}
                    className="flex gap-2 items-center text-[9px] font-mono text-red-400 bg-red-500/5 p-1 px-2 border border-red-500/10"
                  >
            ...
            <Card className="p-6 bg-[#080808] border-white/10">
              ...
              <div
                className={`text-2xl font-black font-mono mb-2 uppercase ${result.complianceStatus.status === 'Compliant' ? 'text-green-400' : 'text-yellow-400'}`}
              >
                {result.complianceStatus.status}
              </div>
              <p className="text-[11px] text-gray-400 font-mono leading-relaxed">
                {result.complianceStatus.summary}
              </p>
            </Card>
