    ...
    <div className="relative w-full aspect-[21/9] bg-[#030303] border border-white/10 rounded-sm overflow-hidden group shadow-2xl">
      ...
      <svg width="100%" height="100%" viewBox="0 0 800 340" className="relative z-10">
        ...
        <g transform="translate(120, 170)">
          ...
          <text y="75" textAnchor="middle" fill="#fff" fontSize="9" fontFamily="JetBrains Mono">
            {result.transactionFlow.from.substring(0, 10)}...
          </text>
        </g>
        ...
        <g transform="translate(400, 170)">
          ...
          <foreignObject x="-25" y="-25" width="50" height="50">
            <div className="w-full h-full flex items-center justify-center">
              <CpuIcon
                className={`w-8 h-8 ${isHighRisk ? 'text-red-500' : 'text-blue-500'} animate-pulse`}
              />
            </div>
          </foreignObject>
          <text
          ...
          <text y="85" textAnchor="middle" fill="#444" fontSize="8" fontFamily="JetBrains Mono">
            Risk_Index: {result.riskLevel}
          </text>
        </g>
        ...
        <g transform="translate(680, 170)">
          ...
          <text y="75" textAnchor="middle" fill="#fff" fontSize="9" fontFamily="JetBrains Mono">
            {result.transactionFlow.to.substring(0, 10)}...
          </text>
        </g>
      ...
      <div className="absolute bottom-3 right-3 flex gap-4 p-2 bg-black/60 border border-white/5 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-1.5">
          <div
            className={`w-1.5 h-1.5 rounded-full ${isHighRisk ? 'bg-red-500' : 'bg-green-500'}`}
          ></div>
      ...
      return (
        <div className="grid grid-cols-1 gap-4">
          {Object.entries(parsed).map(([key, val]) => (
            <div
              key={key}
              className="flex flex-col gap-1 border-l-2 border-white/5 pl-3 group hover:border-blue-500/30 transition-colors"
            >
              <span className="text-[10px] font-mono text-gray-500 uppercase font-bold tracking-tighter group-hover:text-gray-400 transition-colors">
                {key}
              </span>
              <span className="text-xs font-mono text-blue-300 break-all leading-relaxed">
                {String(val)}
              </span>
            </div>
          ))}
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card className="p-4 bg-[#080808] border-white/10">
            <div className="flex justify-between items-center mb-2">
              <label className="block text-[10px] font-bold text-gray-500 uppercase font-mono">
                Raw Payload (JSON)
              </label>
  ...
  "value": "0" 
            ...
            <div className="mt-4">
              <Button
                ...
                disabled={isLoading}
                className="w-full justify-center py-4 bg-white text-black hover:bg-gray-200"
              >
                {isLoading ? (
        ...
        <div className="lg:col-span-8">
          ...
          {result && (
            ...
            >
              {/* Score Card */}
              <Card
                className={`p-6 border flex items-center justify-between overflow-hidden relative ${getRiskStyles(result.riskLevel).bg} ${getRiskStyles(result.riskLevel).border}`}
              >
                <div className="absolute top-0 right-0 p-2 opacity-10">
                  <ThreatIcon className="w-24 h-24" />
                </div>
                <div className="relative z-10">
                  <p className="text-[10px] font-mono uppercase opacity-70 mb-1 tracking-widest font-bold">
                    Threat Assessment
                  </p>
                  <h2
                    className={`text-4xl font-bold uppercase tracking-tighter ${getRiskStyles(result.riskLevel).text}`}
                  >
                    {result.riskLevel}
                  </h2>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-[10px] font-bold text-gray-600 uppercase font-mono tracking-[0.2em]">
                      Security_Vectors_Detected
                    </h3>
                  ...
                  {result.warnings.length === 0 ? (
                    <div className="p-4 border border-green-500/20 bg-green-500/5 rounded-sm flex items-center gap-4 group h-full">
                      ...
                      <div>
                        <p className="text-sm text-green-100 font-bold uppercase tracking-tight">
                          Signal_Clear
                        </p>
                        <p className="text-[11px] text-green-400/70 font-mono">
                          No active exploit signatures or high-risk anomalies detected.
                        </p>
                      </div>
                    </div>
                  ) : (
                        ...
                        >
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1.5">
                              <span className="text-sm font-bold text-red-300 uppercase tracking-tight">
                                {w.title}
                              </span>
                              <span
                                className={`text-[8px] px-2 py-0.5 rounded-sm uppercase font-black tracking-widest ${w.severity === 'High' ? 'bg-red-500 text-white shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-red-500/20 text-red-400'}`}
                              >
                                {w.severity}
                              </span>
                            </div>
