...
const INITIAL_ALERTS: SecurityAlert[] = [
  {
    ...
    severity: 'Warning',
    description:
      'New ERC-1967 transparent proxy deployed on Polygon PoS without verified source code on Polygonscan.',
  ...
  },
  {
    ...
    severity: 'Info',
    description:
      'Unified LxLy bridge batch proof verified by ZK Prover engine with 0 nullifier conflicts.',
  ...
  },
  {
    ...
    severity: 'Critical',
    description:
      'Pre-execution RPC firewall blocked a high-gas frontrunning attempt targeting QuickSwap pool.',
...
const INITIAL_EVENTS: OnChainEvent[] = [
  {
    ...
    type: 'Flash Loan',
    details:
      '1,200,000 POL Flash Loan borrowed from Aave V3. Transaction simulation completed nominal.',
    address: '0x35f...82A1',
    timestamp: new Date().toISOString(),
  },
  {
      ...
      <div className="space-y-1.5 pr-1.5 h-full overflow-y-auto custom-scrollbar pb-4 flex flex-col relative">
        <AnimatePresence initial={false}>
          {filteredItems.map((item, idx) => {
            ...
            const description = isAlert
              ? (item as SecurityAlert).description
              : (item as OnChainEvent).details;
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <span
                        className={`text-[10px] font-black uppercase tracking-tight ${color} truncate`}
                      >
                        {title}
                      </span>
                      <span className="text-[8px] font-mono text-gray-600 font-bold uppercase ml-2">
        ...
        {filteredItems.length === 0 && !isLoading && (
          <div className="flex-1 flex flex-col items-center justify-center opacity-40 py-16 text-center">
            ...
            <span className="text-[9px] font-mono uppercase text-gray-500 tracking-[0.2em]">
      ...
      <div className="p-3 border-b border-white/5 flex flex-col gap-2 bg-[#080808] flex-shrink-0">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div
              className={`w-1.5 h-1.5 rounded-full ${isCongested ? 'bg-red-500' : isPaused ? 'bg-yellow-500' : 'bg-green-500'} animate-flicker shadow-[0_0_8px_currentColor]`}
            ></div>
      ...
      <div className="p-4 flex-1 min-h-0 bg-[#030303] relative overflow-hidden flex flex-col">
        ...
        {isCongested && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-red-950/10 m-2 border border-red-500/20">
            ...
            <span className="text-[8px] text-gray-600 font-mono mt-2 uppercase italic tracking-tighter">
              System is cooling down to prevent lock-out.
            </span>
            <button
          ...
          >
            ...
            <div className="my-3 space-y-3 overflow-y-auto custom-scrollbar flex-1 pr-1">
              <div className="p-3 bg-white/[0.02] border border-white/5 rounded-none space-y-1">
                <span className="text-[7.5px] font-mono text-gray-500 uppercase font-black">
                  Description & Payload
                </span>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 bg-white/[0.02] border border-white/5 space-y-0.5">
                  <span className="text-[7px] font-mono text-gray-500 uppercase font-black">
                    Timestamp
                  </span>
                  <div className="text-[9px] font-mono text-white">
                    {new Date(inspectedItem.timestamp).toLocaleTimeString()}
                  </div>
                </div>
                <div className="p-2.5 bg-white/[0.02] border border-white/5 space-y-0.5">
                  <span className="text-[7px] font-mono text-gray-500 uppercase font-black">
              ...
              <div className="p-2.5 bg-black/80 border border-white/5 font-mono text-[8px] text-gray-400 space-y-1">
                <div className="text-gray-500 font-black uppercase text-[7px]">
                  Simulated Mitigation Code
                </div>
