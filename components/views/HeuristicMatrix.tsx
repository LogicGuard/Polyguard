  ...
  return (
    ...
    <Card className="h-full bg-[#030303] border-white/5 p-0 flex flex-col shadow-2xl relative overflow-hidden rounded-none">
      ...
      <div className="bg-[#0A0A0A] p-3 border-b border-white/5 flex items-center justify-between">
        ...
        <span className="text-[7px] font-mono text-blue-500 uppercase font-black">
          Status_Pulse
        </span>
      </div>
          ...
          {threads.map((thread) => (
            ...
            >
              <div className="flex items-center gap-6">
                ...
                <span className="text-[10px] font-mono text-white font-black w-24 truncate">
                  {thread.protocol}
                </span>
                <div className="flex items-center gap-2 w-24">
                  <div
                    className={`w-1 h-1 rounded-full ${thread.status === 'FLAGGED' ? 'bg-red-500' : 'bg-blue-400'}`}
                  />
                  <span
                    className={`text-[8px] font-mono ${thread.status === 'FLAGGED' ? 'text-red-400' : 'text-gray-400'} uppercase font-bold tracking-tighter`}
                  >
                    {thread.vector}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-4">
                <div className="hidden md:flex flex-col items-end">
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <div
                        key={i}
                        className={`w-0.5 h-2 rounded-full ${i < (thread.score - 70) / 6 ? 'bg-blue-400' : 'bg-white/5'}`}
                      />
                    ))}
                  </div>
                ...
                <div
                  className={`px-2 py-0.5 rounded-none text-[7px] font-black font-mono border ${
                    thread.status === 'FLAGGED'
