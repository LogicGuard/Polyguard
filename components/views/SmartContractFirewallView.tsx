...
const SmartContractFirewallView: React.FC = () => {
  ...
  const [firewallLang, setFirewallLang] = useState<'RUST_EBPF' | 'GO_DAEMON' | 'YUL_GUARD'>(
    'RUST_EBPF'
  );
  ...
  const handleAnalyze = async () => {
    ...
    const { data, error: apiError } = await analyzeTransactionWithFirewall(
      toAddress,
      JSON.stringify({ from: fromAddress, data: dataField })
    );
      ...
      <div className="flex items-center gap-3 mb-6 flex-shrink-0">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 font-mono">
            Deep Packet Inspection // Transaction Shielding
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
        <Card className="flex flex-col p-0 bg-[#080808] h-full overflow-hidden border-white/10">
          ...
          <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
            <div className="group">
              <label className="text-[10px] font-bold text-gray-500 uppercase font-mono mb-1.5 block group-focus-within:text-blue-400 transition-colors">
                Source Address
              </label>
              <Input
                placeholder="0x... (Origin)"
                value={fromAddress}
            ...
            <div className="group flex-1 flex flex-col">
              ...
              <Textarea
                ...
                className="flex-1 bg-black border-white/10 text-xs font-mono resize-none custom-scrollbar"
              />
            </div>
          </div>
        ...
        <Card className="flex flex-col p-0 bg-[#030303] border-white/10 h-full relative overflow-hidden">
          ...
          <div className="flex-1 p-4 overflow-y-auto custom-scrollbar font-mono text-[10px] z-10 space-y-1">
            ...
            {result && (
              ...
              >
                <div
                  className={`p-6 border flex items-start gap-4 ${result.status === 'Allowed' ? 'bg-green-500/5 border-green-500/20' : 'bg-red-500/5 border-red-500/20'}`}
                >
                  {result.status === 'Allowed' ? (
                    <ShieldCheckIcon className="w-10 h-10 text-green-500" />
                  ) : (
                    <ThreatIcon className="w-10 h-10 text-red-500" />
                  )}
                  <div className="flex-1">
                    <h2
                      className={`text-2xl font-black uppercase tracking-widest ${result.status === 'Allowed' ? 'text-green-400' : 'text-red-500'}`}
                    >
                      {result.status}
                    </h2>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div className="p-3 bg-white/[0.02] border border-white/5">
                    <div className="text-[8px] text-gray-600 uppercase font-black mb-1">
                      Threat_Vector
                    </div>
