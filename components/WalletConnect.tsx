...
const WalletConnect: React.FC = () => {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      {error && (
        <p className="text-red-500 text-sm ml-4 fixed bottom-5 right-5 glass-card p-3">{error}</p>
      )}

      {account && (
        <ApprovalManagerModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
      )}
    </div>
  );
};
