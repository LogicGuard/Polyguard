...
const ALERTS = [
  {
    type: 'SCAN',
    msg: 'HEURISTIC_ENGINE: AGGLAYER_NODE_07',
    status: 'ACTIVE',
    color: 'text-polygon-purple',
  },
  {
    type: 'THREAT',
    msg: 'SUSPICIOUS_TX_DETECTION: 0x8a...3f1',
    status: 'BLOCKED',
    color: 'text-red-500',
  },
  {
    type: 'PULSE',
    msg: 'POLYGON_POS_PULSE: 100% UPTIME',
    status: 'NOMINAL',
    color: 'text-green-500',
  },
  {
    type: 'ZK_PROVE',
    msg: 'ZK_PROOF_VERIFICATION: 0xf4...e12',
    status: 'SUCCESS',
    color: 'text-blue-400',
  },
  {
    type: 'FIREWALL',
    msg: 'MEMPOOL_SHIELD_V4: ENGAGED',
    status: 'PROTECTING',
      ...
      <div className="flex items-center whitespace-nowrap animate-ticker group-hover:[animation-play-state:paused]">
        {loopItems.map((alert, i) => (
          <div key={i} className="flex items-center gap-4 px-6 border-r border-white/5">
            ...
            {/* Status Badge */}
            <span
              className={`text-[8px] font-black tracking-widest px-1.5 bg-white/5 ${alert.color}`}
            >
              [{alert.status}]
            </span>
          </div>
