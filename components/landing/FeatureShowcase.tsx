...
const FEATURES = [
  {
    ...
    icon: GlobeIcon,
    description:
      'Aggregate and analyze cross-chain signals to preemptively identify malicious actors before they strike using Polygon AggLayer telemetry.',
    stats: [
      ...
      <div className="h-full flex flex-col justify-between p-4 bg-[#080808] relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="flex items-center justify-between p-2 bg-white/5 border-l-2 border-polygon-purple"
            >
              <span className="text-xs text-gray-300 font-mono">0x8a...3f{i}</span>
              <span className="text-[10px] text-polygon-purple-light bg-polygon-purple/10 px-1 font-bold">
                SIGNAL_DETECTED
              </span>
            </div>
          ))}
  ...
  },
  {
    ...
    icon: AuditorIcon,
    description:
      'Instantaneous static analysis and formal verification of Solidity contracts using fine-tuned Gemini kernels.',
    stats: [
    ...
    mock: (
      <div className="h-full bg-[#0A0A0A] p-4 font-mono text-[10px] text-gray-400 relative">
        ...
        <div className="space-y-1 mt-4">
          <p>
            <span className="text-polygon-purple-light">function</span>{' '}
            <span className="text-white">authorize</span>(address _node){' '}
            <span className="text-polygon-purple-light">public</span> &#123;
          </p>
          <p className="pl-4 text-gray-600 font-bold">// Analysis: Reentrancy Guard Verified</p>
          <p className="pl-4">
            require(nodes[_node].active == <span className="text-polygon-purple-light">true</span>);
          </p>
  ...
  },
  {
    ...
    icon: LayersIcon,
    description:
      'Deep-dive analytics into wallet structures, token distribution, and zk-proof compliance exposure.',
    stats: [
    ...
    mock: (
      <div className="h-full bg-[#080808] p-4 flex flex-col justify-end relative">
        ...
        <div className="relative z-10 flex justify-between items-end">
          <div>
            <div className="text-[10px] text-gray-500 uppercase font-black font-mono">
              Allocation
            </div>
            <div className="text-xl text-white font-mono font-bold tracking-tighter">
              POLYGON_STAKING
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 border-b border-white/5 pb-8">
          <div>
            <div className="inline-block px-3 py-1 bg-polygon-purple/10 border border-polygon-purple/20 rounded-sm mb-4">
              <span className="text-[9px] font-mono text-polygon-purple-light uppercase tracking-widest font-black">
                Core_Capabilities
              </span>
            </div>
            <h2 className="text-3xl md:text-5xl font-black text-white mb-2 tracking-tighter uppercase">
              Modular <span className="text-polygon-purple">Defense</span> Primitives
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4 flex flex-col gap-3">
            {FEATURES.map((feature, idx) => (
              ...
              >
                {selected === idx && (
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-polygon-purple"></div>
                )}
                <div className="flex justify-between items-start mb-4">
                  <span
                    className={`text-[10px] font-mono uppercase tracking-[0.2em] font-black ${selected === idx ? 'text-polygon-purple-light' : 'text-gray-600'}`}
                  >
                    SEC_PRIME_0{idx + 1}
                  </span>
                  <feature.icon
                    className={`w-5 h-5 ${selected === idx ? 'text-white' : 'text-gray-600'}`}
                  />
                </div>
                <h3
                  className={`text-xl font-black mb-2 tracking-tight uppercase ${selected === idx ? 'text-white' : 'text-gray-500'}`}
                >
                  {feature.title}
                </h3>
                <p
                  className={`text-sm font-light leading-relaxed ${selected === idx ? 'text-gray-300' : 'text-gray-600'}`}
                >
                  {feature.description}
                </p>
              </button>
          ...
          <div className="lg:col-span-8 h-[550px] border border-white/10 bg-[#050505] relative overflow-hidden group/viewport">
            ...
            <div className="absolute top-0 left-0 p-3 border-b border-r border-white/10 bg-[#080808] z-20">
              <span className="text-[9px] font-mono text-gray-500 uppercase font-black tracking-widest">
                Viewport_Telemetry // Active
              </span>
            </div>
              ...
              >
                <div className="w-full max-w-2xl aspect-video border border-white/10 bg-[#0C0C0C] shadow-2xl relative">
                  <div className="absolute inset-0 flex flex-col">
                    ...
                    <div className="flex-1 relative">{FEATURES[selected].mock}</div>
                    <div className="h-14 border-t border-white/10 flex divide-x divide-white/5">
                      {FEATURES[selected].stats.map((stat, i) => (
                        <div
                          key={i}
                          className="flex-1 flex flex-col justify-center px-6 bg-[#080808]"
                        >
                          <span className="text-[8px] text-gray-600 uppercase font-black tracking-widest">
                            {stat.label}
                          </span>
                          <span className="text-base text-white font-mono font-bold tracking-tighter">
                            {stat.value}
                          </span>
                        </div>
                      ))}
