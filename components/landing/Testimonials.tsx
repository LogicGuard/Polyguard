...
const REVIEWS = [
  ...
  },
  {
    ...
    text: "PolyGuard's heuristic mapping provides the most granular view of Polygon network health we've deployed to date. A mandatory layer for our stack.",
    status: 'NOMINAL',
    ...
    <section className="py-32 px-6 bg-[#020202] relative overflow-hidden border-b border-white/5">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20">
          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-polygon-purple/10 border border-polygon-purple/20 rounded-sm">
              ...
              <span className="text-[10px] font-mono text-polygon-purple-light uppercase tracking-[0.3em] font-black">
                Field_Intelligence
              </span>
            </div>
            ...
            <p className="text-gray-500 text-sm max-w-md font-light leading-relaxed border-l border-white/10 pl-6 italic">
              Real-time telemetry and feedback from active security units deployed across the
              AggLayer.
            </p>
          </div>
          <div className="hidden md:block text-right font-mono">
            <div className="text-[10px] text-gray-700 uppercase tracking-widest font-black mb-1">
              Source: Verified_Logs
            </div>
            <div className="text-[12px] text-polygon-purple-light font-bold">
              100% CRYPTOGRAPHIC_INTEGRITY
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-4">
                <div className="flex flex-col">
                  <span className="text-[9px] font-mono text-polygon-purple font-black tracking-widest leading-none mb-1">
                    {review.id}
                  </span>
              ...
              <div className="mb-10 min-h-[120px] relative">
                <span className="absolute -left-4 top-0 text-polygon-purple/30 font-mono text-lg font-black">
                  {'>'}
                </span>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-white/5 bg-gradient-to-t from-white/[0.01] to-transparent -mx-8 px-8 -mb-8 pb-8">
                <div>
                  <div className="text-[13px] font-black text-white uppercase tracking-wider group-hover:text-polygon-purple-light transition-colors">
                    {review.name}
                  </div>
                  <div className="text-[9px] text-gray-600 font-mono uppercase tracking-tight font-bold">
                    {review.role}
                  </div>
                </div>
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-none text-[9px] font-black font-mono border transition-all duration-500 ${
        ...
        <div className="mt-20 pt-10 border-t border-white/5 flex flex-wrap justify-center gap-x-12 gap-y-6 opacity-20 hover:opacity-100 transition-opacity duration-1000">
          {['TX_VERIFIED', 'AUDIT_COMPLETE', 'THREAT_NEUTRALIZED', 'UPLINK_STABLE'].map(
            (status, idx) => (
              <div key={idx} className="flex items-center gap-3">
                <div className="w-1 h-1 bg-polygon-purple rounded-full animate-pulse"></div>
                <span className="text-[9px] font-mono text-gray-500 font-black tracking-[0.4em] uppercase">
                  {status}
                </span>
              </div>
            )
