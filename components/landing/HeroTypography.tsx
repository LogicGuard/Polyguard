...
const HeroTypography: React.FC<HeroTypographyProps> = ({
        ...
        <>
          {parts.map((part, i) => {
            ...
            return isHighlight ? (
              <span
                key={i}
                className="text-polygon-purple font-black drop-shadow-[0_0_15px_rgba(123,63,228,0.4)]"
              >
                {part}
              </span>
            ) : (
  ...
  return (
    <div className={`leading-[1] tracking-tight ${className}`}>
      {renderContent()}
      {!isFinished && (
        <span className="inline-block w-[0.15em] h-[0.8em] ml-1 align-middle bg-polygon-purple shadow-[0_0_12px_rgba(123,63,228,0.8)]"></span>
      )}
    </div>
  );
};
