...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setUptime((99.998 + Math.random() * 0.001).toFixed(4));
    }, 3000);
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-20">
          <div className="md:col-span-4 space-y-8">
            <div
              ...
              className="cursor-pointer group/brand"
            >
            ...
            <ul className="space-y-3">
              {footerLinks.resources.map((link) => (
                <li key={link.label}>
                  ...
                  >
                    <span className="opacity-0 group-hover/link:opacity-100 transition-opacity text-polygon-purple">
                      {'>'}
                    </span>
                    {link.label}
                  </button>
              ...
              {footerLinks.protocol.map((link) => (
                <li key={link.label}>
                  <button
                    ...
                    className="text-[10px] text-gray-500 font-mono font-bold uppercase tracking-wider hover:text-white hover:translate-x-1 transition-all flex items-center gap-2 group/link text-left"
                  >
                    <span className="opacity-0 group-hover/link:opacity-100 transition-opacity text-blue-500">
                      {'>'}
                    </span>
                    {link.label}
                  </button>
          ...
          <div className="md:col-span-4 space-y-6">
            <div>
              <h4 className="font-black text-white mb-2 uppercase text-[9px] tracking-[0.3em] font-mono">
                Intelligence Feed
              </h4>
              <p className="text-[9px] text-gray-600 font-light uppercase tracking-widest leading-relaxed">
                Subscribe to real-time cryptographic vulnerability alerts.
              </p>
            </div>
        ...
        <div className="pt-10 border-t border-white/5 flex flex-col lg:flex-row justify-between items-center gap-6 text-[8px] text-gray-700 font-mono font-black tracking-[0.2em]">
          ...
          <div className="flex flex-col lg:items-end items-center gap-1">
            <p className="uppercase opacity-60">© 2025 POLYGUARD DEFENSE CLUSTER.</p>
            <div className="flex items-center gap-3 text-[7px] text-gray-800">
              <span>UID: PG-3.2-PRO</span>
